...
class CronTaskRegistry
{
     ...
     *
     * @param string $key Clave del tipo "cron.pickup_reminder".
     ...
     *
     * @param string $command Nombre del comando, p. ej. "app:purge-usage-hits".
     * @return array<string, mixed>|null
     */
     ...
     *   salta: es lo que permite congelar el listado a mano un lunes con el
     *   cron caído.
     ...
     * está caída, y tratarla como tal llenaría la pantalla de falsas alarmas.
     * Sin ninguna ejecución registrada NO se considera retraso, porque no hay
     * referencia desde la que medir (la pantalla lo muestra como "sin registro
     * todavía", que es la verdad).
     *
     * @param string       $key     Clave de la tarea.
     ...
     *
     * Se calcula aquí y no en la plantilla porque no hay instalado el filtro
     * `time_diff` de twig/extra-bundle, y una fecha absoluta sola no grita: lo
     * que salta a la vista de un cron caído es el "hace 16 días".
     *
     ...
     *
     * Es la validación de coherencia del manifiesto en su caso más real: el
     * recordatorio de recogida sólo lee cestas ya congeladas, así que con el
     * congelado semanal apagado corre en verde sin avisar a nadie. Sin este
     * cruce, ese fallo es invisible.
     *
     * @param string $key Clave de la tarea.
