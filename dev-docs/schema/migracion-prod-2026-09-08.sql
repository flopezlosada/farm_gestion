-- ============================================================================
-- MIGRACIÓN DE ESQUEMA A PRODUCCIÓN · 8 de septiembre de 2026
--
-- LO QUE LE FALTA A PRODUCCIÓN para el código que hay hoy en `main`.
-- Producción ya tiene la PARTE A del fichero del 4 de septiembre (las tablas de
-- voluntariado, avisos, grupo de consumo y destinatarios del listado); le falta
-- todo lo posterior.
--
-- CÓMO SE HA SABIDO QUÉ FALTA. Se cargó el dump real de producción
-- (`gestioncsa.sql`) en una base local y se le preguntó a Doctrine con el código
-- de `main`: `doctrine:schema:update --dump-sql`. Lo que sale, quitando el drift
-- viejo de tipos y nombres de índice, es exactamente esto.
--
-- ⚠️ LAS PARTES 1 Y 2 VAN ANTES DE SUBIR EL CÓDIGO. Voluntariado deja de abrir
-- en cuanto Doctrine espera una columna que no está, y el calendario de reparto
-- entero revienta sin la columna del traslado sumando.
--
...
-- módulo. Pasó en local el 4 de septiembre, en ese orden exacto.
--
-- ⚠️ NO ES IDEMPOTENTE. Una vez por entorno. Repetirla falla en la primera clave
-- ajena duplicada y deja el resto sin aplicar.
--
-- ⚠️ En phpMyAdmin, comprobar a mano que la base seleccionada es la de verdad:
-- `DATABASE()` no es de fiar allí, y en este hosting hay una base
-- `csavegadejarama` que NO es producción. Prueba rápida:
-- `SELECT COUNT(*) FROM weekly_basket;` debe dar miles de filas.
--
-- LA MIGRACIÓN DE DATOS DE VOLUNTARIADO NO TOCA NADA. Comprobado sobre el dump
-- real: `volunteer_offer`, `volunteer_signup`, `volunteer_call`,
-- `volunteer_category` y `volunteer_event` están todas a CERO filas en
-- producción. Los `INSERT ... SELECT` y los `UPDATE` de la parte 1 se ejecutan
-- igual —son inofensivos y así el fichero es el mismo que se validó— pero no van
-- a mover ninguna fila.
--
-- Y NO HAY NINGÚN TRASLADO SUMANDO QUE MARCAR. En staging hay uno; aquí la
-- consulta devuelve cero filas, porque el único que hubo (shift 116) es de una
-- semana que ya pasó y ésos no se pueden recuperar de la papelera.
-- ============================================================================


-- ============================================================================
-- PARTE 1 · ANTES del código — turnos, montaje, rutina y traslado sumando
--
-- Es la PARTE B del fichero del 4 de septiembre, sin cambios: ya se validó
-- entera sobre una copia real y salió idéntica al esquema que espera el código.
-- ============================================================================
...
-- ============================================================================
-- PARTE 2 · ANTES del código — una cuenta por socix
--
-- ESTO NO ESTÁ EN NINGÚN FICHERO ANTERIOR: es drift que nunca llegó a
-- producción. `User::$partner` es una relación OneToOne, así que Doctrine espera
-- un índice ÚNICO donde producción tiene el índice normal de la clave ajena.
-- Staging ya lo tiene (por haberse reconstruido con `schema:create`), y por eso
-- sólo aparece al comparar contra producción.
--
-- COMPROBADO ANTES DE PROPONERLO: en el dump real de producción no hay ningún
-- socix con dos cuentas, así que el índice único entra sin conflicto. La
-- consulta de comprobación va primero, por si eso cambia de aquí al despliegue:
-- si devuelve alguna fila, PARAR y resolver el duplicado a mano.
-- ============================================================================

SELECT partner_id, COUNT(*) AS cuentas
FROM fos_user
WHERE partner_id IS NOT NULL
GROUP BY partner_id
HAVING COUNT(*) > 1;

-- Sólo si la consulta de arriba devuelve CERO filas:
ALTER TABLE fos_user DROP INDEX FK_957A64799393F8FE,
                     ADD UNIQUE INDEX UNIQ_957A64799393F8FE (partner_id);
ALTER TABLE fos_user DROP FOREIGN KEY FK_957A64799393F8FE;
ALTER TABLE fos_user ADD CONSTRAINT FK_957A64799393F8FE FOREIGN KEY (partner_id)
